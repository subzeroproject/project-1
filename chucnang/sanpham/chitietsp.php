...
?>
            ...
            <div id="comment" class="col-md-8 col-sm-9 col-xs-12">
                ...
                <form method="post" action="index.php?page_layout=chitietsp&id_sp=<?php echo $id_sp; ?>">
                  ...
                  <div class="form-group">
                    ...
                    <textarea name="binh_luan" required="" class="form-control" placeholder="Học viện Công nghệ Vietpro"></textarea>
                  </div>
                  <button name="submit" type="submit" class="btn btn-default">Bình luận</button>
                </form>
            </div>
            <?php
			if(isset($_GET['page'])){
			...
			$sqlBl="SELECT * FROM blsanpham WHERE id_sp=$id_sp ORDER BY id_bl DESC LIMIT $perRow,$rowsPerPage";
			$queryBl=mysqli_query($connect,$sqlBl);
			
		$totalRows=mysqli_num_rows(mysqli_query($connect, "SELECT * FROM blsanpham WHERE id_sp=$id_sp"));
